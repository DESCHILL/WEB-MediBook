...
import Doctor_calendar from '../components/doctor_calendar.jsx';
...
function doctor_content({path}) {
    ...
    return <><h1>{patients?'Danh sách bệnh nhân':'Quản lý lịch hẹn'}</h1><Request_state {...state}/>{patients ? <div className="table_scroll"><table className="workspace_table"><thead><tr><th>Mã BN</th><th>Tên bệnh nhân</th><th>Tuổi</th><th>Thời gian hẹn</th><th>Nhập kết quả khám</th><th>Trạng thái</th></tr></thead><tbody>{items.map((item)=><tr key={item.lich_hen_id}><td>{item.benh_nhan_id}</td><td>{item.ho_ten}</td><td>{item.tuoi??'—'}</td><td>{item.bat_dau_luc.slice(0,10)} · {format_time(item.bat_dau_luc)}</td><td><Result_field item={item} refresh={state.retry}/></td><td>{item.trang_thai}</td></tr>)}</tbody></table>{state.data&&items.length===0&&<p className="request_state">Chưa có bệnh nhân đặt lịch.</p>}</div> : <><label className="calendar_date">Tuần có ngày <input type="date" value={date} onChange={(e)=>set_date(e.target.value)}/></label><Doctor_calendar date={date} items={items}/></> }</>;
}
