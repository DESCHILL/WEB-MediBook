...
import {prepare_email_verification} from '../services/email_content_service.js';

export function create_auth_repository(database) {
    async function create_patient(input) {
        const pool = await database.get_pool();
        const result = await bind_verification(pool.request(),input.verification??prepare_email_verification()).input('email', input.email).input('ho_ten', input.ho_ten).input('mat_khau_hash', input.mat_khau_hash).query(`
            SET XACT_ABORT ON;
            BEGIN TRY
                BEGIN TRANSACTION;
                ...
                DECLARE @email_account_id bigint=@tai_khoan_id;
    ...
    async function find_by_email(email) {
        const pool = await database.get_pool();
        const result = await pool.request().input('email', email).query(`SELECT CONVERT(varchar(20),tai_khoan_id) AS tai_khoan_id,email,ho_ten,vai_tro,mat_khau_hash,hoat_dong,email_xac_minh_luc FROM dbo.TaiKhoan WHERE email=@email;`);
        return result.recordset[0];
    }
    ...
    async function find_session(session_id, account_id) {
        ...
        const result = await pool.request().input('session_id', session_id).input('account_id', account_id).query(`
            SELECT CONVERT(varchar(20),a.tai_khoan_id) AS tai_khoan_id,a.email,a.ho_ten,a.vai_tro,a.hoat_dong,a.email_xac_minh_luc
